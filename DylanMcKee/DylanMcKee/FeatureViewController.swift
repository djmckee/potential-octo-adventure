...
class FeatureViewController: UIViewController, UIScrollViewDelegate {
    ...
    override func viewDidLoad() {
        ...
        let totalScrollWidth:CGFloat = (imageScrollView.frame.size.width * CGFloat(data.images.count))
        ...
        for image:UIImage in data.images {
            ...
            newImageView.frame = CGRectMake(newXPosition, 0, imageScrollView.frame.size.width, imageScrollView.frame.size.height)
    ...
    @IBAction func pageControlChanged(sender: AnyObject) -> () {
        ...
        let scrollPosition = (CGFloat(pageControl.currentPage) * (imageScrollView.frame.size.width))
